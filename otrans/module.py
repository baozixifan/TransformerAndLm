...
class LRPEPositionalEncoding(PositionalEncoding):
    """Positional encoding."""
    ...
    def __init__(self, d_model, dropout_rate, max_len=5000):
        ...
        """
        self.length = max_len
        super(LRPEPositionalEncoding, self).__init__(d_model, dropout_rate, max_len=5000)
    ...
    def extend_pe(self, x):
        ...
        position = torch.arange(0, x.size(1), dtype=torch.float32).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, self.d_model, 2, dtype=torch.float32) *
                             -(math.log(float(self.length)) / self.d_model))
        pe[:, 0::2] = torch.sin(position * div_term)
        pe[:, 1::2] = torch.cos(position * div_term)
        pe = pe.unsqueeze(0)
        self.pe = pe.to(device=x.device, dtype=x.dtype)
...
class LDPEPositionalEncoding(nn.Module):
    """Positional encoding."""
    ...
    def __init__(self, d_model, dropout_rate, max_len=5000):
        ...
        """
        super(LDPEPositionalEncoding, self).__init__()
        self.d_model = d_model
        self.xscale = math.sqrt(self.d_model)
        self.dropout = nn.Dropout(p=dropout_rate)
        self.pe = None
    ...
    def extend_pe(self, x):
        ...
        div_term = torch.exp(torch.arange(0, self.d_model, 2, dtype=torch.float32) *
                             -(math.log(10000.0) / self.d_model))
        pe[:, 0::2] = torch.sin((self.length - position) * div_term)
        pe[:, 1::2] = torch.cos((self.length - position) * div_term)
        pe = pe.unsqueeze(0)
        self.pe = pe.to(device=x.device, dtype=x.dtype)
