...
def main(args):
    ...
    shutil.copy(args.config, expdir)
    ...
    # build optimizer
    if params['train']['finetuning'] == True:
        print('Now is finetuning, please notice learning rate!!!!!')
        optimizer = FinetuningOptimizer(model)
    else:
    ...
    dev_loader = FeatureLoader(
        params, 'dev', shuffle=params['train']['shuffle'],
        ngpu=args.ngpu, mode=args.parallel_mode)
