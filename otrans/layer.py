...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, attention_heads, d_model, linear_units, slf_attn_dropout_rate, 
                 ...
                 concat_after=False, activation='relu', drop_head_rate=0):
        super(TransformerEncoderLayer, self).__init__()
        self.self_attn = MultiHeadedAttentionPdrophead(attention_heads, d_model, slf_attn_dropout_rate, p=drop_head_rate)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, attention_heads, d_model, linear_units, slf_attn_dropout_rate, src_attn_dropout_rate, 
                 ffn_dropout_rate, residual_dropout_rate, normalize_before=True, concat_after=False,
                 activation='relu', drop_head_rate=0):
        super(TransformerDecoderLayer, self).__init__()

        self.self_attn = MultiHeadedAttentionPdrophead(attention_heads, d_model, slf_attn_dropout_rate, p=drop_head_rate)
        self.src_attn = MultiHeadedAttentionPdrophead(attention_heads, d_model, src_attn_dropout_rate, p=drop_head_rate)
        self.feed_forward = PositionwiseFeedForward(d_model, linear_units, ffn_dropout_rate, activation)
